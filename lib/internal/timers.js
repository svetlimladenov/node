...
// item = Timer, which has the _ontimeout = callback,
// getLibuvNow - uses a binding to call timers.cc which uses libuv - uv_now, and returns the now
function insert(item, msecs, start = getLibuvNow()) {
  // Truncate so that accuracy of sub-millisecond timers is not assumed.
  msecs = MathTrunc(msecs);
  item._idleStart = start; // set idleStart as a number returned by uv_now - 14056505
  ...
  if (list === undefined) {
    ...
    if (nextExpiry > expiry) {
      scheduleTimer(msecs); // a cpp binddin in timers.cc, which internally calls uv_timer_start in the env.cc
      nextExpiry = expiry;
    }
  }
